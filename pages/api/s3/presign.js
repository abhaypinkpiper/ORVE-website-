...
function getS3Client() {
  ...
  if (!region) throw new Error('Missing S3_REGION (or AWS_REGION/AWS_DEFAULT_REGION)');
...
function makePublicUrl({ bucket, region, key, publicBaseUrl }) {
...
async function getPresignedUrl({ fileName, contentType }) {
  ...
  const publicUrl = makePublicUrl({ bucket, region, key, publicBaseUrl });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (typeof size === 'number' && Number.isFinite(size) && size > 25 * 1024 * 1024) {
    ...
    return res.status(500).json({ error: e?.message || 'Error generating pre-signed URL' });
  }
}
