    ...
    <div style={{ minHeight: '100vh', paddingTop: '80px', background: '#F5EFE0' }}>
      ...
      <section style={{ maxWidth: '600px', margin: '0 auto', padding: '80px 40px' }}>
        <div style={{ background: '#FFFDF7', padding: '50px 40px', borderTop: '3px solid', borderImage: 'linear-gradient(135deg, #C9A84C, #A07830) 1' }}>
          ...
          {error && <p style={{ marginTop: '16px', color: '#A07830', fontSize: '0.8rem', letterSpacing: '1px' }}>⚠️ {error}</p>}
            ...
            <div style={{ background: '#FFFDF7', padding: '40px', marginBottom: '24px' }}>
              ...
              <div style={{ marginTop: '30px' }}>
                ...
                <div style={{ position: 'relative' }}>
                  <div style={{ position: 'absolute', top: '20px', left: '20px', right: '20px', height: '2px', background: '#E2D5BE' }} />
                  <div
                    style={{
                      position: 'absolute',
                      top: '20px',
                    ...
                    {STATUS_STEPS.map((step, i) => {
                      ...
                      return (
                        <div key={step} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px', flex: 1 }}>
                          ...
                          >
                            {done ? <span style={{ color: 'white', fontSize: '1rem' }}>✓</span> : <span style={{ color: '#B8A88A', fontSize: '0.7rem' }}>{i + 1}</span>}
                          </div>
                          <p style={{ fontSize: '0.55rem', letterSpacing: '1px', color: done ? '#C9A84C' : '#B8A88A', fontWeight: done ? 600 : 400, textAlign: 'center', textTransform: 'uppercase' }}>
                            {step}
                          </p>
                        </div>
                      );
              ...
              <div style={{ marginTop: '40px', padding: '20px', background: '#F5EFE0', borderLeft: '3px solid #C9A84C' }}>
                ...
                <p style={{ fontSize: '0.75rem', color: '#A07830', letterSpacing: '1px', marginTop: '8px' }}>📅 Order placed: {result.date}</p>
              </div>
            </div>

            <a
              href={`https://wa.me/917977459392?text=Hi%20ORVÉ!%20I%20need%20help%20with%20my%20order%20${result.id}`}
              target="_blank"
              rel="noreferrer"
              className="btn-gold"
              style={{ display: 'block', textAlign: 'center', fontSize: '0.7rem', letterSpacing: '3px' }}
            >
