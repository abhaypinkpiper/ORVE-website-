...
export default function ShopPage() {
  ...
  useEffect(() => {
    ...
    const category = typeof router.query.category === 'string' && router.query.category.trim() !== '' ? router.query.category : 'All';
    setActiveCategory(category);
  }, [router.isReady, router.query.category]);

  const categories = ['All', 'Rings', 'Necklaces', 'Earrings', 'Bracelets', 'Sets'];

  let filtered = products.filter(p => (activeCategory === 'All' || p.category === activeCategory) && (search === '' || p.name.toLowerCase().includes(search.toLowerCase())));
  ...
  return (
    <div style={{ minHeight: '100vh', paddingTop: '100px', background: '#F5EFE0' }}>
      ...
      <div style={{ background: 'linear-gradient(160deg, #2C1A0E 0%, #4A3020 100%)', padding: '80px 40px', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: '0.8rem', color: '#B8A88A', letterSpacing: '3px', marginTop: '12px' }}>{filtered.length} pieces curated for you</p>
      </div>
      ...
      <div style={{ maxWidth: '1300px', margin: '0 auto', padding: '60px 40px' }}>
        {productsLoading && products.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(270px, 1fr))', gap: '28px' }}>
            {filtered.map(p => (
              <ProductCard key={p.id} product={p} productIndex={indexById.get(p.id)} />
            ))}
          </div>
        )}
      </div>
      ...
      <div style={{ background: '#EDE0C8', padding: '80px 40px', textAlign: 'center' }}>
        <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.5rem', fontWeight: 300, letterSpacing: '3px', color: '#2C1A0E', marginBottom: '8px' }}>
