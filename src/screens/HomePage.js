  ...
  return (
    ...
    >
      <div style={{ position: 'relative', overflow: 'hidden', aspectRatio: '3/4' }}>
        {productHref ? (
          <Link href={productHref} style={{ display: 'block' }} onClick={(e) => e.stopPropagation()}>
            <img
              src={product.image}
              alt={product.name}
              style={{
...
export default function HomePage() {
  ...
  const featured = (products.filter(p => p.featured).length > 0 ? products.filter(p => p.featured) : products).slice(
    0,
    6
  );
  ...
  return (
    <div>
      ...
      <div style={{ background: 'linear-gradient(135deg, #C9A84C, #A07830)', padding: '16px 0', overflow: 'hidden' }}>
        ...
        >
          {Array(8)
            .fill(['✦ ANTI-TARNISH', '✦ LUXURY QUALITY', '✦ FREE DELIVERY', '✦ ELEGANCE YOU WEAR', '✦ HANDCRAFTED', '✦ ORVE.CO.IN'])
            .flat()
            .map((text, i) => (
              <span key={i} style={{ color: 'white', fontSize: '0.65rem', letterSpacing: '3px', fontWeight: 600 }}>
                {text}
              </span>
            ))}
        </div>
      ...
      <section style={{ padding: '100px 40px', background: '#F5EFE0', maxWidth: '1300px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '24px' }}>
          {categories.map(cat => (
            <Link key={cat.name} href={`/shop?category=${encodeURIComponent(cat.name)}`} className="cat-card" style={{ textDecoration: 'none', display: 'block' }}>
              <div style={{ overflow: 'hidden', aspectRatio: '3/4', position: 'relative' }}>
                ...
                <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(44,26,14,0.7) 0%, transparent 60%)' }} />
      ...
      <section style={{ padding: '80px 40px', background: '#EDE0C8' }}>
        <div style={{ maxWidth: '1300px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '60px' }}>
            ...
            <h2 className="section-title" style={{ color: '#2C1A0E' }}>
              Featured Pieces
            </h2>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '24px' }}>
            {featured.map(product => (
              <ProductCard key={product.id} product={product} productIndex={products.findIndex(p => p.id === product.id)} />
            ))}
          </div>
      ...
      >
        ...
        <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center', position: 'relative', zIndex: 2 }}>
          <p style={{ fontSize: '0.6rem', letterSpacing: '6px', color: '#C9A84C', fontWeight: 600, textTransform: 'uppercase', marginBottom: '20px' }}>
            Our Story
          </p>
          <h2
            style={{
          ...
          >
            ORVÉ was created for women who believe that luxury shouldn't compromise on longevity. Every piece in our
            collection is crafted with anti-tarnish technology, ensuring your jewellery looks as radiant on day 300 as it
            did on day one.
          </p>
          <Link href="/about" className="btn-outline" style={{ borderColor: '#C9A84C', color: '#C9A84C', fontSize: '0.7rem', letterSpacing: '3px' }}>
            Read Our Story
          </Link>
        </div>
      </section>
      ...
      <section style={{ padding: '100px 40px', background: '#F5EFE0' }}>
        <div style={{ maxWidth: '1300px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <h2 className="section-title" style={{ color: '#2C1A0E', marginBottom: '8px' }}>
            @ORVE.jewels
          </h2>
          <a
            href="https://instagram.com/ORVE.jewels"
            target="_blank"
            rel="noreferrer"
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '3px' }}>
            {[
              ...
              <a key={i} href="https://instagram.com/ORVE.jewels" target="_blank" rel="noreferrer" style={{ display: 'block', aspectRatio: '1', overflow: 'hidden', position: 'relative' }}>
                <img
                  src={img}
                  alt={`ORVÉ Instagram ${i + 1}`}
                ...
                >
                  <span
                    style={{ color: 'white', fontSize: '1.5rem', opacity: 0 }}
