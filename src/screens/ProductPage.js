        ...
        <div className="product-grid" style={{ display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: '36px' }}>
          <div>
            <div style={{ background: '#FFFDF7', overflow: 'hidden' }}>
              {activeImageUrl ? (
                <img src={activeImageUrl} alt={product.name} style={{ width: '100%', height: 'auto', display: 'block' }} />
              ) : (
            ...
            {videoUrls.length > 0 && (
              <div style={{ marginTop: '22px' }}>
                <p style={{ fontSize: '0.6rem', letterSpacing: '4px', color: '#A07830', fontWeight: 700, textTransform: 'uppercase', marginBottom: '10px' }}>Videos</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {videoUrls.map((url, i) => (
                    <a key={`${url}-${i}`} href={url} target="_blank" rel="noreferrer" style={{ textDecoration: 'none', color: '#2C1A0E', fontSize: '0.85rem', letterSpacing: '1px' }}>
                      Watch video {i + 1} →
                    </a>
                  ))}
                </div>
