...
export default function UploadPage() {
  ...
  const startUpload = async () => {
    setError(null);
    setBusy(true);
    setUploads(files.map((f) => ({ name: f.name, size: f.size, progress: 0, status: 'queued', publicUrl: '', key: '' })));
    ...
    try {
      for (let i = 0; i < files.length; i += 1) {
        ...
        setUploads((prev) =>
          prev.map((u, idx) =>
            idx === i
              ? { ...u, status: 'uploading', progress: 0, key: presign.key || '', publicUrl: presign.publicUrl || '' }
              : u
          )
        );
  ...
  return (
    <div style={{ minHeight: '100vh', paddingTop: '90px', background: '#F5EFE0' }}>
      <div style={{ background: 'linear-gradient(160deg, #2C1A0E 0%, #4A3020 100%)', padding: '70px 40px', textAlign: 'center' }}>
        ...
        <p style={{ color: '#B8A88A', fontSize: '0.85rem', letterSpacing: '2px', marginTop: '12px' }}>Upload directly to S3 using a presigned URL</p>
      </div>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '70px 40px' }}>
        <div style={{ background: '#FFFDF7', padding: '40px', borderTop: '3px solid', borderImage: 'linear-gradient(135deg, #C9A84C, #A07830) 1' }}>
          <p style={{ fontSize: '0.65rem', letterSpacing: '3px', color: '#C9A84C', fontWeight: 600, textTransform: 'uppercase', marginBottom: '18px' }}>Select Files</p>
        ...
        {uploads.length > 0 && (
          <div style={{ marginTop: '26px', background: '#FFFDF7', padding: '30px' }}>
            <p style={{ fontSize: '0.65rem', letterSpacing: '3px', color: '#C9A84C', fontWeight: 600, textTransform: 'uppercase', marginBottom: '18px' }}>Uploads</p>
